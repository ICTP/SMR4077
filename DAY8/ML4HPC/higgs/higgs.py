...
def main():
    ...
    y_preds_custom=y_preds_custom.round().astype(int)
    wait([y_preds, y_valid, y_preds_custom])
    print("\ny_preds.head():\n", y_preds.head(), flush=True)
    print("\ny_preds.shape:\n",  y_preds.shape,  flush=True)
    print("\ny_preds_custom.head():\n", y_preds_custom.head(), flush=True)
    print("\ny_preds_custom.shape:\n",  y_preds_custom.shape,  flush=True)
    print("\ny_valid.head():\n", y_valid.head(), flush=True)
    print("\ny_valid.shape:\n",  y_valid.shape,  flush=True)
    score  = accuracy_score(y_preds, y_valid)
    score_custom = accuracy_score(y_preds_custom, y_valid)
    print(f"\nmodel accuracy: {score}\n", flush=True)
    print(f"\ncustom model accuracy: {score_custom}\n", flush=True)
    y_valid = y_valid.compute()
    y_preds = y_preds.compute()
    y_preds_custom = y_preds_custom.compute()
    print("\nAfter calling compute():\n", flush=True)
    print("\ny_preds.shape:\n",  y_preds.shape,  flush=True)
    print("\ny_preds_custom.shape:\n",  y_preds_custom.shape,  flush=True)
    print("\ny_valid.shape:\n",  y_valid.shape,  flush=True)
    print("\nmodel results:\n", flush=True)
    report(y_valid.to_cupy(), y_preds.to_cupy(), 'cm_matrix.png')
    print("\ncustom model results:\n", flush=True)
    report(y_valid.to_cupy(), y_preds_custom.to_cupy(), 'cm_matrix_custom.png')
    roc_auc = roc_auc_score(y_valid.to_cupy(), y_preds.to_cupy())
    roc_auc_custom = roc_auc_score(y_valid.to_cupy(), y_preds_custom.to_cupy())
