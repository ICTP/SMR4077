...
for NODE in ${NODES}; do
        ...
        for (( GPU_ID=1; GPU_ID<=${SLURM_NTASKS_PER_NODE};GPU_ID++ )); do
            ...
            --nthreads=${SLURM_CPUS_PER_TASK} --interface "ib0" --protocol "ucx" --device-memory-limit "60GB" \
            --enable-nvlink --enable-infiniband --enable-rdmacm --rmm-pool-size "50GB" --death-timeout 3600  &
        done
done
